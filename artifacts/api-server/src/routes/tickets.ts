...
import { autoAssignForDepartment, autoAssignFromDepartments } from "../lib/autoAssign";
...
router.get("/tickets", authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (search) {
      conditions.push(ilike(ticketsTable.subject, `%${search}%`));
    }
    ...
    const [userRows, deptRows, commentCounts] = await Promise.all([
      ...
      tickets.length > 0 ? db
        .select({ ticketId: commentsTable.ticketId, count: sql<number>`count(*)::int` })
        .from(commentsTable)
        .where(inArray(commentsTable.ticketId, tickets.map((t) => t.id)))
  ...
  try {
    ...
    if (userRole !== "super_admin") {
      const [rolePerm] = await db.select({ canCreateTicket: rolePermissionsTable.canCreateTicket })
        .from(rolePermissionsTable).where(eq(rolePermissionsTable.role, userRole as any)).limit(1);
    ...
    if (tagList2.includes("employment-verification")) {
      // Fetch dept IDs for HR + Admin + Operations
      ...
      // Fetch dept IDs for Admin + HR + BGV
      const poolDepts = await db.select({ id: departmentsTable.id }).from(departmentsTable)
    ...
    const [ticket] = await db
      .insert(ticketsTable)
      .values({ ticketNumber, subject, description, priority, status, departmentId: departmentId ?? null, assigneeId: resolvedAssigneeId, createdById, tags, slaDeadline, raisedForName: raisedForName ?? null, raisedForEmail: raisedForEmail ?? null, ccEmails: Array.isArray(ccEmails) ? ccEmails : [] } as any)
      .returning();
...
router.get("/tickets/:ticketId", authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const userRows = userIds.length > 0
      ? await db.select({ id: usersTable.id, name: usersTable.name, avatar: usersTable.avatar }).from(usersTable).where(inArray(usersTable.id, userIds))
      : [];
    const usersMap = new Map(userRows.map((u) => [u.id, u.name]));
    ...
    const historyAuthorIds = [...new Set(history.map((h) => h.changedById))];
    const allIds = [...new Set([...commentAuthorIds, ...historyAuthorIds])].filter((id) => !usersMap.has(id));
    ...
    res.json({
      ...
      history: history.map((h) => ({
        ...
        changedByName: usersMap.get(h.changedById) ?? "Unknown",
    ...
    if (historyEntries.length > 0) {
      await Promise.all(
        historyEntries.map((entry) =>
          db.insert(ticketHistoryTable).values({ ticketId, changedById, ...entry })
        )
      );
    }
...
router.get("/tickets/:ticketId/comments", authMiddleware, async (req, res) => {
  try {
    ...
    const authorRows = authorIds.length > 0
      ? await db.select({ id: usersTable.id, name: usersTable.name, avatar: usersTable.avatar }).from(usersTable).where(inArray(usersTable.id, authorIds))
      : [];
