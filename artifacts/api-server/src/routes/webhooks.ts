  ...
  try {
    ...
    for (const ep of matching) {
      try {
        ...
        logger.info({ event, url: ep.url, status: res.status }, "Outgoing webhook dispatched");
      } catch (err) {
        logger.warn({ event, url: ep.url, err }, "Outgoing webhook failed");
      }
    }
  } catch (err) {
    logger.error({ err }, "dispatchOutgoingWebhooks error");
  }
}
...
router.post("/config/regenerate-secret", authMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  logger.info("Inbound webhook secret regenerated");
