...
router.get("/dashboard/stats", authMiddleware, async (req, res) => {
  try {
    ...
    const [
      ...
      db.select({ count: sql<number>`count(*)::int` }).from(ticketsTable).where(
        and(eq(ticketsTable.status, "resolved"), sql`${ticketsTable.updatedAt} >= ${todayStart}`)
      ),
    ]);

    const avgRow = await db.select({
      avg: sql<number>`COALESCE(AVG(EXTRACT(EPOCH FROM (${ticketsTable.updatedAt} - ${ticketsTable.createdAt})) / 3600), 0)::float`,
    }).from(ticketsTable).where(eq(ticketsTable.status, "resolved"));
...
router.get("/dashboard/department-stats", authMiddleware, async (req, res) => {
  try {
    ...
    req.log.error({ err }, "Department stats error");
    res.status(500).json({ error: "Internal Server Error" });
  }
});
  ...
  try {
    ...
    res.json(
      tickets.map((t) => ({
        ...
        assigneeName: t.assigneeId ? usersMap.get(t.assigneeId) ?? null : null,
        createdById: t.createdById,
        createdByName: usersMap.get(t.createdById) ?? "Unknown",
        tags: t.tags ?? [],
    ...
    req.log.error({ err }, "Recent tickets error");
    res.status(500).json({ error: "Internal Server Error" });
  }
});
  ...
  try {
    const rows = await db.execute(sql`
      SELECT 
        ...
        u.avatar,
        COALESCE(SUM(CASE WHEN t.status = 'resolved' THEN 1 ELSE 0 END), 0)::int as resolved_count,
        COALESCE(AVG(CASE WHEN t.status = 'resolved' 
          THEN EXTRACT(EPOCH FROM (t.updated_at - t.created_at)) / 3600 
          ELSE NULL END), 0)::float as avg_resolution_hours,
    ...
    res.json(
      (rows.rows as Array<{
        ...
        avatar: r.avatar,
        resolvedCount: r.resolved_count,
        avgResolutionHours: Math.round(r.avg_resolution_hours * 10) / 10,
        activeTickets: r.active_tickets,
        satisfactionScore: null,
      }))
    );
  } catch (err) {
    req.log.error({ err }, "Agent performance error");
    res.status(500).json({ error: "Internal Server Error" });
  }
});
...
router.get("/dashboard/sla-overview", authMiddleware, async (req, res) => {
  try {
    ...
    const [totalRow] = await db
      ...
      .from(ticketsTable)
      .where(sql`${ticketsTable.slaDeadline} IS NOT NULL`);

    const [compliantRow] = await db
      .select({ count: sql<number>`count(*)::int` })
      .from(ticketsTable)
      .where(
        and(
          sql`${ticketsTable.slaDeadline} IS NOT NULL`,
          eq(ticketsTable.slaBreached, false)
        )
      );
    ...
    const [breachedRow] = await db
      .select({ count: sql<number>`count(*)::int` })
      .from(ticketsTable)
      .where(eq(ticketsTable.slaBreached, true));
    ...
    const [atRiskRow] = await db
      ...
      .where(
        and(
          sql`${ticketsTable.slaDeadline} IS NOT NULL`,
          eq(ticketsTable.slaBreached, false),
          sql`${ticketsTable.slaDeadline} <= ${atRiskThreshold}`,
    ...
    const breached = breachedRow?.count ?? 0;
    const atRisk = atRiskRow?.count ?? 0;
    ...
    req.log.error({ err }, "SLA overview error");
    res.status(500).json({ error: "Internal Server Error" });
  }
});
