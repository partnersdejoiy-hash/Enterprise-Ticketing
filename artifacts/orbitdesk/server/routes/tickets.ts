...
router.post("/tickets", authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const [ticket] = await db.insert(ticketsTable).values({ ticketNumber, subject, description, priority, status, departmentId: departmentId ?? null, assigneeId: assigneeId ?? null, createdById, tags, slaDeadline, raisedForName: raisedForName ?? null, raisedForEmail: raisedForEmail ?? null } as any).returning();
